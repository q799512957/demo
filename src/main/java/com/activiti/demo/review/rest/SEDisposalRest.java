...
@RestController
@RequestMapping("/sEDisposal")
@Api(tags = {"证据等级保存处理决定书"})
public class SEDisposalRest {

    @Autowired
    private SEDisposalService service;
    ...
    @ResponseBody
    public ResultBean update(@RequestBody SEDisposal seDisposal) {
        boolean r = service.update(seDisposal);
        ResultBean resultBean = new ResultBean();
        if (r){
            resultBean.setCode("200");
    ...
    @ResponseBody
    public ResultBean insert(@RequestBody SEDisposal seDisposal) {
        ResultBean resultBean = new ResultBean();
        boolean r = service.insert(seDisposal);
        if (r){
            resultBean.setCode("200");
