...
@RestController
@RequestMapping("/sOCorrect")
@Api(tags = {"责令整改通知书"})
public class SOCorrectRest {

    @Autowired
    private SOCorrectService service;
    ...
    @ResponseBody
    public ResultBean update(@RequestBody SOCorrect soCorrect) {
        boolean r = service.update(soCorrect);
        ResultBean resultBean = new ResultBean();
        if (r){
            resultBean.setCode("200");
    ...
    @ResponseBody
    public ResultBean insert(@RequestBody SOCorrect soCorrect) {
        ResultBean resultBean = new ResultBean();
        boolean r = service.insert(soCorrect);
        if (r){
            resultBean.setCode("200");
