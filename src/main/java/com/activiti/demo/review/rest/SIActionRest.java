...
@RestController
@RequestMapping("/sIAction")
@Api(tags = {"违法行为通知书"})
public class SIActionRest {

    @Autowired
    private SIActionService service;
    ...
    @ResponseBody
    public ResultBean update(@RequestBody SIAction siAction) {
        boolean r = service.update(siAction);
        ResultBean resultBean = new ResultBean();
        if (r){
            resultBean.setCode("200");
    ...
    @ResponseBody
    public ResultBean insert(@RequestBody SIAction siAction) {
        ResultBean resultBean = new ResultBean();
        boolean r = service.insert(siAction);
        if (r){
            resultBean.setCode("200");
