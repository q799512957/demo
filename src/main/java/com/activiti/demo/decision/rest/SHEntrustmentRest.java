...
@RestController
@RequestMapping("/sHEntrustment")
@Api(tags = {"听证委托书"})
public class SHEntrustmentRest {

    @Autowired
    private SHEntrustmentService service;
    ...
    @ResponseBody
    public ResultBean update(@RequestBody SHEntrustment shEntrustment) {
        boolean r = service.update(shEntrustment);
        ResultBean resultBean = new ResultBean();
        if (r){
            resultBean.setCode("200");
    ...
    @ResponseBody
    public ResultBean insert(@RequestBody SHEntrustment shEntrustment) {
        ResultBean resultBean = new ResultBean();
        boolean r = service.insert(shEntrustment);
        if (r){
            resultBean.setCode("200");
