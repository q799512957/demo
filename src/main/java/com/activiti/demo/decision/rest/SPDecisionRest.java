...
@RestController
@RequestMapping("/sPDecision")
@Api(tags = {"行政处罚决定书"})
public class SPDecisionRest {

    @Autowired
    private SPDecisionService service;
    ...
    @ResponseBody
    public ResultBean update(@RequestBody SPDecision spDecision) {
        boolean r = service.update(spDecision);
        ResultBean resultBean = new ResultBean();
        if (r){
            resultBean.setCode("200");
    ...
    @ResponseBody
    public ResultBean insert(@RequestBody SPDecision spDecision) {
        ResultBean resultBean = new ResultBean();
        boolean r = service.insert(spDecision);
        if (r){
            resultBean.setCode("200");
