...
@RestController
@RequestMapping("/sHReport")
@Api(tags = {"听证报告"})
public class SHReportRest {

    @Autowired
    private SHReportService service;
    ...
    @ResponseBody
    public ResultBean update(@RequestBody SHReport shReport) {
        boolean r = service.update(shReport);
        ResultBean resultBean = new ResultBean();
        if (r){
            resultBean.setCode("200");
    ...
    @ResponseBody
    public ResultBean insert(@RequestBody SHReport shReport) {
        ResultBean resultBean = new ResultBean();
        boolean r = service.insert(shReport);
        if (r){
            resultBean.setCode("200");
